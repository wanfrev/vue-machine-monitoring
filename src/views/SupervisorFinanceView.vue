...
import { getMachineDailyIncome, getMachines } from "@/api/client";
...
async function loadMachines() {
  ...
  const filtered = filterMachinesForRole(Array.isArray(rows) ? rows : [], {
    role: currentRole.value,
    assignedMachineIds: assignedMachineIds.value,
  });
  machines.value = filtered.map((m) => ({
    id: String(m.id),
    name: m.name,
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p
            ...
            :class="isDark() ? 'text-zinc-400' : 'text-slate-500'"
          >
            Suma del total de monedas de tus maquinas
          </p>
        </div>
        <label class="text-xs">
